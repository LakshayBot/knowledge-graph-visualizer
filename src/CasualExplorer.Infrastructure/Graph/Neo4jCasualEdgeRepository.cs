...
namespace CasualExplorer.Infrastructure.Graph;

/// <summary>
/// Neo4j Cypher implementation of <see cref="ICasualEdgeRepository"/>.
/// Casual edges are stored as <c>CAUSED</c> relationships between <c>EventNode</c> nodes.
/// </summary>
internal sealed class Neo4jCasualEdgeRepository : ICasualEdgeRepository
{
    private readonly Neo4jContext _neo4j;

    /// <summary>Initialises the repository with the shared Neo4j context.</summary>
    public Neo4jCasualEdgeRepository(Neo4jContext neo4j) => _neo4j = neo4j;

    // ── Queries ───────────────────────────────────────────────────────────────

    /// <inheritdoc />
    public async Task<CasualEdge?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public async void Update(CasualEdge edge)
    {
        await using var session = _neo4j.GetWriteSession();
        await session.RunAsync(
    ...
    /// <inheritdoc />
    public async void Delete(CasualEdge edge)
    {
        await using var session = _neo4j.GetWriteSession();
        await session.RunAsync(
    ...
    private static CasualEdge MapRelationship(IRelationship rel, Guid fromId, Guid toId)
    {
        var relationshipType = Enum.Parse<CasualRelationshipType>(rel["relationshipType"].As<string>());
        ...
        var edge = CasualEdge.Create(fromId, toId, relationshipType, strength, perspective, explanation, isContested);

        // Overwrite the auto-generated Id with the stored one via reflection.
        var idProp = typeof(CasualExplorer.Domain.Common.BaseEntity)
