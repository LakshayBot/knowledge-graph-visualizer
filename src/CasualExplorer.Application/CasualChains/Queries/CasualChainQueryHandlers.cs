...
/// <summary>Handles <see cref="GetCasualChainQuery"/>.</summary>
public sealed class GetCasualChainQueryHandler : IRequestHandler<GetCasualChainQuery, CasualGraphDto>
{
    private readonly ICasualChainRepository _chainRepo;
    private readonly IEventNodeRepository _nodeRepo;
    private readonly ICasualEdgeRepository _edgeRepo;

    /// <summary>Initialises the handler.</summary>
    public GetCasualChainQueryHandler(
        ICasualChainRepository chainRepo,
    ...
    /// <inheritdoc />
    public async Task<CasualGraphDto> Handle(
        GetCasualChainQuery request,
        CancellationToken cancellationToken)
    {
        var depth = Math.Clamp(request.Depth, 1, 6);

        var chain = await _chainRepo.GetByIdAsync(request.ChainId, cancellationToken)
            ?? throw new NotFoundException(nameof(CasualChain), request.ChainId);

        var visitedNodes = new Dictionary<Guid, EventNode>();
        var visitedEdges = new List<Domain.Entities.CasualEdge>();
        ...
        return new CasualGraphDto(graphNodes, graphEdges, metadata);
    }
    ...
    private async Task BfsAsync(
        ...
        List<Domain.Entities.CasualEdge> edges,
    ...
    private static GraphEdgeDto BuildGraphEdge(Domain.Entities.CasualEdge edge) =>
...
public sealed class ExpandChainNodeQueryHandler : IRequestHandler<ExpandChainNodeQuery, CasualGraphDto>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (expansion.SuggestedNodes.Count == 0)
            return new CasualGraphDto([], [], new ChainMetadataDto(
                chain.Id, chain.Title, chain.Domain.ToString(),
                chain.NodeCount, chain.ViewCount, chain.LastUpdatedAt));
        ...
        // ── Persist casual edges to Neo4j ──────────────────────────
        var persistedEdges = new List<CasualEdge>();
        for (int i = 0; i < expansion.SuggestedNodes.Count; i++)
        {
            ...
            var edge = CasualEdge.Create(fromId, toId, relType, 0.5m, perspective,
        ...
        await _pgStore.BulkAddCasualEdgesAsync(persistedEdges, cancellationToken);
    ...
    private static CasualRelationshipType MapRelationshipType(string? type) => type?.ToUpperInvariant() switch
    {
        "CAUSES"         => CasualRelationshipType.DirectlyCaused,
        "CONTRIBUTES_TO" => CasualRelationshipType.ContributedTo,
        "ENABLES"        => CasualRelationshipType.EnabledConditionsFor,
        "PREVENTS"       => CasualRelationshipType.Contested,
    ...
    private static void UpdateGraphSnapshot(
        ...
        IReadOnlyList<Domain.Entities.CasualEdge> newEdges)
    {
...
public sealed class GetInitialChainQueryHandler : IRequestHandler<GetInitialChainQuery, CasualGraphDto>
{
    private readonly IEventNodeRepository _nodeRepo;
    private readonly ICasualChainRepository _chainRepo;
    private readonly ICasualEdgeRepository _edgeRepo;
...
public sealed class GetChainScopedGraphQueryHandler
    : IRequestHandler<GetChainScopedGraphQuery, CasualGraphDto>
{
    private readonly ICasualChainRepository _chainRepo;
    private readonly IEventNodeRepository _nodeRepo;
    private readonly ICasualEdgeRepository _edgeRepo;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (needsRebuild && chainNodeIds.Count > 0)
        {
            ...
            var allEdges = new List<Domain.Entities.CasualEdge>();
            foreach (var nid in chainNodeIds)
            {
        ...
        if (string.IsNullOrWhiteSpace(chain.GraphSnapshot))
            return new CasualGraphDto([], [], metadata);
        ...
        return new CasualGraphDto(nodes, edges, metadata);
    }

    private static void UpdateGraphSnapshotFromNodes(
        CasualChain chain,
        IReadOnlyList<EventNode> allNodes,
        IReadOnlyList<Domain.Entities.CasualEdge> allEdges)
    {
        using var ms = new System.IO.MemoryStream();
