...
/// <summary>
/// Represents a named, curated sequence of casually linked events — a navigable
/// path through the casual knowledge graph rooted at a single event.
/// </summary>
public sealed class CasualChain : BaseEntity
{
    // ── Public properties ─────────────────────────────────────────────────────
    ...
    /// <summary>Gets the human-readable title of this casual chain.</summary>
    ...
    /// <summary>
    /// Creates a new <see cref="CasualChain"/> with an initial node count of 1
    /// (the root event).
    /// </summary>
    ...
    /// <returns>A newly constructed <see cref="CasualChain"/>.</returns>
    /// <exception cref="ArgumentException">
    /// Thrown when <paramref name="title"/> is null or whitespace.
    /// </exception>
    public static CasualChain Create(Guid rootEventId, string title, EventDomain domain)
    {
        if (string.IsNullOrWhiteSpace(title))
            throw new ArgumentException("Title must not be null or whitespace.", nameof(title));

        var chain = new CasualChain
        {
    ...
    public void DecrementNodeCount()
    {
        if (NodeCount <= 1)
            throw new InvalidOperationException(
                "Cannot remove the root node from a casual chain.");
    ...
    /// <summary>
    /// Records a view of this casual chain by incrementing the view counter.
    /// </summary>
    ...
    /// <summary>
    /// Updates the title of this casual chain.
    /// </summary>
