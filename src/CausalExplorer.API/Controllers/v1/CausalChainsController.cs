...
/// <summary>
/// Operations for browsing, creating, expanding, and saving causal chains.
/// </summary>
[ApiVersion("1.0")]
[EnableRateLimiting(RateLimitPolicies.Authenticated)]
public sealed class CausalChainsController : ApiControllerBase
{
    private readonly ICurrentUserService _currentUser;

    /// <summary>Initialises the controller.</summary>
    public CausalChainsController(ICurrentUserService currentUser)
        => _currentUser = currentUser;

    /// <summary>
    /// Returns the full graph structure for a causal chain, optionally filtered by
    ...
    public async Task<IActionResult> GetCausalChain(
        Guid id,
        ...
        CancellationToken cancellationToken  = default)
    {
        ...
        var result = await Sender.Send(
            new GetCausalChainQuery(id, perspective, depth), cancellationToken);
        return Ok(result);
    }
    ...
    [ProducesResponseType(typeof(CausalChainSummaryDto), StatusCodes.Status201Created)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    public async Task<IActionResult> CreateCausalChain(
        [FromBody] CreateCausalChainCommand command,
        CancellationToken cancellationToken = default)
    {
    ...
    /// <summary>
    /// Saves a causal chain to the current user's personal library.
    /// Requires an authenticated user.
    /// </summary>
